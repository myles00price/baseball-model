...
history at **myles00price.github.io/baseball-model/board.html**. Everything
on the board — record, P&L, closing-line value, edge buckets, prop ledgers —
is regenerated nightly from the files in this repo. Nothing is hand-typed.
...
and team strikeout rate. Retrained weekly on a rolling multi-season window;
the current coefficients and walk-forward accuracy are shown live in the
board's MODEL BIOLOGY section. Probabilities are clamped to 22–78 and no
...
| Grading + record | `check_results.py`, `daily_results_notify.py`, `weekly_report.py` | nightly 10:30 PM |
...
| Retrains | `weekly_retrain.py`, `props_retrain.py` | weekly |
...
- `ODDS_API_KEY` comes from the environment. It is never committed.
